...
> 性质:**竞品领域参考**,用于校准我们自己的保本/成本建模。**非交付物**。
> 注:我们 v1 的财务计算权威实现是**前端 TS 计算引擎**(`frontend/src/lib/breakeven/`,见 CLAUDE.md §2 计算归属),后端不算账。本文件仅作公式 / 默认值的领域校准来源。
...
- **全静态手填**:所有输入(单产、现金价、每项成本)都是用户在单元格里手敲;无任何外部数据源,玉米带「平均值」占位。
  - 注:**v1 我们同样是手填**(现金价由农户手填,不接实时数据源 —— CLAUDE.md §0)。v1 的差异化不在数据接入,而在**录入体验(低摩擦 / 可复用上季)+ 交互式敏感性热力图 + 不崩的边界守卫**。把数据缝合成实时本地行情是 v1 OUT、以后再说。
- **无本地化**:没有 ZIP / region 概念,没有本地粮库挂牌价或 basis;现金价(`Input!E18`)纯手打。底部仅给 MN FINBIN / ISU / IL FarmDoc 三个外链让用户「自己去查」,没有数据接入。
  - 注:v1 也不做 basis / ZIP 行情;`GET /defaults` 仅按地区给**默认值**兜底,不是实时价。
- **无预警 / 无信号**:没有卖出时机、basis 预警、轮作建议;敏感性矩阵只是静态展示,不会主动触发任何决策提示。也没有时间序列(只有单期 2026 快照)。
  - 注:v1 同样不做预警 / 信号 / 驾驶舱(均 OUT);但 v1 的敏感性矩阵是**交互式**的(价格 × 单产可实时拖动重算),这是相对竞品静态网格的改进点。
